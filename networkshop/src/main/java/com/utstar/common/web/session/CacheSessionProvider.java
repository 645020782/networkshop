package com.utstar.common.web.session;
...
/*		HttpSession session = request.getSession();//true    Cookie JSESSIONID
		...
		//保存远程了
		RedisClusterUtil.getInstance().set(getSessionId(request,response), session, 0, "",expiry*60);
	}
	...
	public Serializable getAttribute(HttpServletRequest request,HttpServletResponse response, String name) {
		byte[] bs = RedisClusterUtil.getInstance().get(getSessionId(request,response).getBytes(),0);
		Map<String,Serializable> session = null;
		if(bs != null){
			session = (Map<String, Serializable>) SerializeUtil.unserialize(bs);
		}
		if(null != session){
	...
	public void logout(HttpServletRequest request,HttpServletResponse response) {
		if(RedisClusterUtil.getInstance().exists(getSessionId(request, response).getBytes(), 0)){
			RedisClusterUtil.getInstance().del(getSessionId(request, response).getBytes(), 0);
		}
		//清理Cookie
