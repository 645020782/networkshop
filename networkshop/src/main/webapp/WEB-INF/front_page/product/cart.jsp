...
//结算
function trueBuy(){
 	window.location.href = "/control/buyer/trueBuy.shtml";
}
//消空购物车
function clearCart(){
	window.location.href = "/control/shopping/clearCart.shtml";
}
//+
function addProductAmount(skuId,buyLimit){
	...
	//跟后台进行交互
	//$("#num" + skuId).val(++num);
	window.location.href = "/control/shopping/buyCart.shtml?skuId=" + skuId + "&amount=1" ;
}
...
function delProduct(skuId){
	if(confirm("你确定要删除吗?")){
		window.location.href = "/control/shopping/deleteItem.shtml?skuId=" + skuId;
	}
}
</script>
</head>
		...
		<div class="ofc pb40">
			...
			<th>操作</th>
				...
				<tr>
					...
					<td><a onclick="subProductAmount(${item.sku.skuId })" class="inb arr" title="减" href="javascript:void(0);">-</a>
					<input type="text" id="num${item.sku.skuId }" readonly="readonly" value="${item.amount }" name="" size="1" class="txts">
					<a onclick="addProductAmount(${item.sku.skuId },${item.sku.skuUpperLimit })" class="inb arr" title="加" href="javascript:void(0);">+</a>
					</td>
					<td class="blue"><a onclick="delProduct(${item.sku.skuId })" title="删除" href="javascript:void(0);">删除</a></td>
				</tr>
				</c:forEach>
			...
			<div class="page">
				<span class="l">
					<input type="button" onclick="window.open('/product/detail.shtml?productId=274')" class="hand btn100x26c" title="继续购物" value="继续购物">
