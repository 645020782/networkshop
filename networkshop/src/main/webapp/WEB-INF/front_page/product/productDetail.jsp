...
function buy(productId){
	window.location.href='/control/shopping/buyCart.shtml?skuId=' + skuId + "&amount=" + $("#num").val() + "&buyLimit=" + buyLimit + "&productId=" + productId;
		...
		<ul class="uls form">
			...
			<li><label>选择颜色：</label>
				<div id="colors" class="pre spec">
					...
					<a onclick="colorToRed(this,${color.colorId})" href="javascript:void(0)" title="${color.name }" class="changToWhite"><img width="25" height="25" data-img="1" src="/res/img/pic/ppp00.jpg" alt="${color.name } "><i>${color.name }</i></a>
					</c:forEach>
				</div>
			</li>
			...
			<li class="submit"><input type="button" value="" class="hand btn138x40" onclick="buy('${product.productId}');"/><input type="button" value="" class="hand btn138x40b" onclick="addCart()"/></li>
		</ul>
	</div>
</div>
<div class="w ofc mt">
