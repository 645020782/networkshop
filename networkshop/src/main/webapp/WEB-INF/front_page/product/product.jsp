...
//登陆
function login(){
	window.location.href = "/control/shopping/login.shtml?returnUrl=" + window.location.href;
}
//我的订单
function myOrder(){
...
<div class="w ofc">
	...
	<div class="r wr">
			...
			<ul class="uls filter">
				...
				<li><b>品牌：</b><p>
					...
					<c:forEach items="${brands}" var="brand">
						<a href="javascript:void(0);" title="${brand.brandName }" class="here">${brand.brandName }</a>
					</c:forEach>
				...
				<li><b>类型：</b><p>
					...
					<c:forEach	items="${types}" var="type">
						<a href="javascript:void(0);" title="${type.name }">${type.name }</a>
					</c:forEach>
				</p></li>
				<li><b>材质：</b><p>
			...
			<div class="mt ofc">
				<ul class="uls i_150x150 x4_150x150b">
					<c:forEach items="${pagination.rows}" var="row">
						<li>
						<a href="/html/product/${row.productId}.html" title="瑜伽服" target="_blank" class="pic"><img src="${row.img.allUrl }" alt="瑜伽服" /></a>
						<dl>
							<!-- dt 10个文字+... -->
							<dt><a href="/control/product/detail.shtml?productId=${row.productId}" title="${row.name }" target="_blank">${row.name }</a></dt>
							<!-- dt 25个文字+... -->
							<dd class="h40">${row.name }</dd>
		...
		<a href="/control/product/display/list.shtml?&amp;beginPage=${pagination.beginPage}">首页</a>
		<c:if test="${pagination.currentPage gt 1}">
			<a href="/control/product/display/list.shtml?&amp;beginPage=${pagination.currentPage-1}">上一页</a>
		</c:if>
		<c:forEach begin="${pagination.beginPage}" end="${pagination.endPage}" var="pageNum">
			<a href="/control/product/display/list.shtml?&amp;beginPage=${pageNum}"><font size="2"><c:out value="${pageNum}"/></font></a>
		</c:forEach>
		<c:if test="${pagination.currentPage lt pagination.endPage}">
			<a href="/control/product/display/list.shtml?&amp;beginPage=${pagination.currentPage+1}"><font size="2">下一页</font></a>
		</c:if>
		<a href="/control/product/display/list.shtml?&amp;beginPage=${pagination.endPage}"><font size="2">尾页</font></a>
		共<var>${pagination.endPage}</var>页 到第<input type="text" size="3" id="PAGENO"/>页 <input type="button" onclick="javascript:window.location.href = '/control/product/display/list.shtml?&amp;isShow=0&amp;pageNo=' + $('#PAGENO').val() " value="确定" class="hand btn60x20" id="skip"/>
</span></div>
	</div>
</div>
</div>
